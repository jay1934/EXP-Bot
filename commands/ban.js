  ...
  async execute(message, [id, ...reason]) {
    ...
    const embed = (dm) =>
      new MessageEmbed()
        ...
        .setTitle(`${dm ? 'You were' : `${member.user.tag} was`} banned!`)
        .addField('Reason:', reason.join(' ') || 'No reason provided')
        .setFooter(
          `Banned by ${message.author.tag} • ${message.guild.name}`,
          message.member.displayAvatarURL
        );

    member
      .ban({ reason: reason.join(' ') || 'No reason provided' })
      .then(() => {
      ...
      .catch(() => err(":x: I can't ban the member!"));
  },
};
